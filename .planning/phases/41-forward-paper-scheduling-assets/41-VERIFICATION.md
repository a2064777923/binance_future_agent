...
| 7 | Paper observation can scan wider auto-hot symbols without changing live allowlist. | VERIFIED | CLI/config/deploy tests cover `BFA_FORWARD_PAPER_*`, `--auto-hot-symbols`, and top-40 paper service args; server run selected 40 symbols and created no order intents. |
...
| `python -m unittest discover -s tests` | Passed, 322 tests after auto-hot follow-up |
| Server focused auto-hot/deploy tests | Passed, 30 tests |
| Server `python -m unittest discover -s tests` | Passed, 322 tests |
| Server manual `ops forward-paper-run --auto-hot-symbols --top-n 40 ...` | Selected 40 symbols, generated 15 paper signals, skipped 25 |
| Server DB count check | `paper_signals=23`, `paper_outcomes=0`, `order_intents=18` |
